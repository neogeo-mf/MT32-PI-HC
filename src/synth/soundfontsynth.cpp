...
// Declare method
const char* CSoundFontSynth::GetCurrentSoundFontName() const {
...
extern "C" {
    ...
    FILE* fluid_file_open(const char* path, const char** errMsg) {
        ...
        if (!pFile && errMsg) *errMsg = "Failed to open file";
        return pFile;
    }
    ...
    int safe_fseek(void* fd, fluid_long_long_t ofs, int whence) {
        FIL* pFile = static_cast<FIL*>(fd);
        switch (whence) {
            case SEEK_CUR: ofs += f_tell(pFile); break;
            case SEEK_END: ofs += f_size(pFile); break;
            default: break;
        }
